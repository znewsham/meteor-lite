...
async function updateDependenciesForArch(actualPackages, arch) {
  const map = await generateGlobals(actualPackages.map(dep => `@meteor/${dep}`), arch);
...
export default async function convertPackagesToNodeModulesForApp({
  ...
  await Promise.all(allPackages.map(packageName => convertPackage(packageName, outputDirectory, ...directories)));
