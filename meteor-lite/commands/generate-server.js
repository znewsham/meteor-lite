...
import {
  baseFolder, baseBuildFolder, ensureBuildDirectory, readPackageJson, listFilesInDir,
...
function queueForBuild(buildRoot, queue) {
  return {
    ...
    async setup(build) {
      ...
      build.onResolve({ filter: /.*/ }, async ({ kind, path: filePath, resolveDir, importer, ...args }) => {
        const watchFiles = [];
        const watchDirs = ['/home/zacknewsham/Sites/npm-meteor-converter/test-packages/node_modules/@meteor/ddp-server'];
